...
========================
Exp 2 (Cross-Domain Comparison) + Exp 3 (Cross-Method Agreement) analysis script.
...
Output: results/cross_domain_results.json  +  console summary
"""
...
SENTINEL = -20.72326584  # Sentinel value used to fill upper-triangular entries in KL matrices
...
def load_method1(domain: str) -> list[dict]:
    """
    Load black-box resampling results. Returns a per-problem list, each item:
      {
        'problem_id': str,
...
def load_method2(domain: str) -> dict[str, list[float]]:
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            score = float(raw) if raw.strip() else 0.0  # post-convergence rows may be empty
            result[pid].append((sidx, score))

    # Sort by sentence_idx, return score-only lists
    return {pid: [s for _, s in sorted(v)] for pid, v in result.items()}


# Fixed forward window size for Method 3 scoring.
# Only the next k=3 sentences after masking are averaged, eliminating the
# end-of-chain position bias that arises when averaging over the entire
# remaining chain (shorter tails for later sentences inflate their scores).
M3_WINDOW = 3


def load_method3(domain: str) -> dict[str, np.ndarray]:
    """
    Load causal masking results and compute per-sentence importance scores.
    Returns {problem_id: per_sentence_causal_score (1D array)}.

    Per-sentence score = mean KL divergence over the next M3_WINDOW sentences
    after masking sentence i. Using a fixed window (instead of averaging all
    downstream sentences) removes the systematic bias where late-chain
    sentences score artificially high due to a shrinking evaluation window.
    """
    ...
    for fname in os.listdir(kl_dir):
        ...
        for i in range(N):
            ...
            col = mat[i + 1:j_end, i]  # Only the next M3_WINDOW sentences
...
def load_method3_matrix(domain: str) -> dict[str, np.ndarray]:
    """Load raw KL matrices {problem_id: N x N array} for Exp 2c topology analysis."""
    kl_dir = os.path.join(BASE_DIR, "kl_results", domain, MODEL_NAME, "correct")
    result = {}
    for fname in os.listdir(kl_dir):
        if not fname.endswith("_kl.npy"):
...
def permutation_test_2samp(a: np.ndarray, b: np.ndarray,
    ...
    """Two-sample permutation test, returns two-tailed p-value."""
...
def bootstrap_jsd_ci(tags_a: list[str], tags_b: list[str],
    ...
    """Bootstrap CI for JSD (resampling over chunk label lists)."""
    rng = np.random.default_rng(42)
    jsds = []
    for _ in range(n_boot):
...
def cliffs_delta_interp(d: float) -> str:
    """Interpret Cliff's delta magnitude (Cliff, 1993 thresholds)."""
    ad = abs(d)
    if ad < 0.147:
        return "negligible"
    elif ad < 0.33:
...
def normalize_tag(tag: str) -> str:
    """Normalize tag format to underscore-separated lowercase."""
    return tag.lower().replace(" ", "_").replace("-", "_")


def exp2a_gini_jsd(m1_gpqa: list, m1_math: list) -> dict:
    """Exp 2a: Gini coefficient + Jensen-Shannon Divergence."""
...
# ──────────────────────────────────────────────────────────────────────────────
# 4. Exp 2b -- Attention Entropy (Shannon entropy of receiver head scores)
# ──────────────────────────────────────────────────────────────────────────────
...
def exp2b_attention_entropy(m2_gpqa: dict, m2_math: dict,
    ...
    Proxy metric: treat the per-problem receiver_head_score distribution as an
    attention distribution and compute its Shannon entropy.
    High entropy = multi-broadcast (attention spread across many sentences).
    Low entropy  = single-hop (attention focused on a few sentences).

    Note: can be replaced with raw attention matrix entropy if available.
    """
    def problem_entropy(scores: list[float]) -> float:
        arr = np.array(scores, dtype=float)
        arr = arr - arr.min() + 1e-9  # Shift to non-negative
    ...
    return {
        ...
        "interpretation": (
            "High entropy = multi-broadcast (GPQA expected higher), "
            "low entropy = single-hop (Math expected lower). "
            "This proxy is based on receiver_head_score distribution; "
            "replace with raw attention matrix entropy for a stricter measure."
        )
    }


# ──────────────────────────────────────────────────────────────────────────────
# 5. Exp 2c -- Causal Topology (Reach Ratio + Anchor Position Distribution)
# ──────────────────────────────────────────────────────────────────────────────

def causal_reach_ratio(mat: np.ndarray, threshold_frac: float = 0.5) -> float:
    """
    Causal reach ratio = total long-range influence / total short-range influence.

    For the lower triangle of the KL matrix (sentence i -> sentence j, j > i):
    - Distance d = j - i
    - Threshold = ceil(chain_length * threshold_frac)
    - Long-range: d > threshold; short-range: d <= threshold
    Sentinel values are excluded; only valid causal influences are counted.
    """
    N = mat.shape[0]
    ...
    for i in range(N - 1):
        for j in range(i + 1, N):
            ...
            if val <= SENTINEL + 1:
                continue
            # Shift by sentinel to get a positive "relative strength" value
            effective = val - SENTINEL
            if j - i > threshold:
                far_sum += effective
            else:
                near_sum += effective
...
def anchor_locations(scores: np.ndarray, z_threshold: float = 1.0) -> list[float]:
    ...
    (position / chain_length, in [0, 1]).

    A sentence is an anchor if its z-score > z_threshold.
    """
    if len(scores) == 0:
        return []
...
def exp2c_causal_topology(m1_gpqa: list, m1_math: list,
    ...
    # Mann-Whitney U test for positional shift
...
def kendalls_w(rankings: np.ndarray) -> float:
    ...
    rankings: shape (k, n) -- k raters (methods), n items (sentences).
    """
    k, n = rankings.shape
    mean_rank = (n + 1) / 2
...
def pairwise_kendall_tau(r1, r2, n_boot: int = 5000):
    """Kendall's tau between two ranking sequences, with bootstrap CI."""
...
def topk_agreement(s1, s2, s3, k: int) -> dict:
    """
    Top-k set overlap (Jaccard-style).

    Returns pairwise overlap ratios and three-way overlap ratio for the
    top-k highest-scoring sentences identified by each method.
    """
    n = len(s1)
    k = min(k, n)
...
def align_three_methods(m1_problem: dict,
                         m2_all: dict,
                         m3_all: dict) -> Optional[tuple]:
    """
    Align scores from three methods on the same problem, returning (s1, s2, s3).

    Alignment strategy:
    - Method 1 has N chunks
    - Method 2 has N-1 sentences (excludes the last sentence)
    ...
    # Drop last sentence from Method 1 to align with Methods 2 & 3
...
def exp3_method_agreement(m1: list, m2_all: dict, m3_all: dict,
    ...
    """Exp 3: Compute cross-method agreement metrics for a single domain."""
    Ws = []
    ...
    for prob in m1:
        ...
        # Pairwise tau (point estimates only; bootstrap done on aggregated values)
    ...
    # Bootstrap CI for each tau list (treating per-problem taus as independent)
    def tau_ci(lst):
        if len(lst) < 5:
            return None
...
def exp3_cross_domain_W(res_gpqa: dict, res_math: dict) -> dict:
    """Exp 3: Compare Kendall's W across domains (Mann-Whitney U + Cliff's delta)."""
...
def print_summary(results: dict):
    ...
    print(f"  Permutation p={fmt(g['permutation_p'])}  Cliff's d={fmt(g['cliffs_delta'])} [{g['effect_size']}]")
    ...
    print(f"  Permutation p={fmt(ae['permutation_p'])}  Cliff's d={fmt(ae['cliffs_delta'])} [{ae['effect_size']}]")
    ...
    print(f"  Permutation p={fmt(rr['permutation_p'])}  Cliff's d={fmt(rr['cliffs_delta'])} [{rr['effect_size']}]")
    ...
    print(f"  Mann-Whitney U p={fmt(al['mann_whitney_p'])}  Cliff's d={fmt(al['cliffs_delta'])} [{al['effect_size']}]")
    ...
    print("\n" + "=" * 70)
    print("Exp 3 -- Pairwise Kendall's tau")
    print("=" * 70)
    for dom in ["gpqa", "math"]:
        pt = results["exp3"][dom]["pairwise_tau"]
        print(f"  {dom.upper()}:")
        for pair, info in pt.items():
            print(f"    {pair}: tau_mean={fmt(info['mean'])}  CI={info['ci95']}")
    ...
    print(f"  Mann-Whitney U p={fmt(cd['mann_whitney_p'])}  Cliff's d={fmt(cd['cliffs_delta'])} [{cd['effect_size']}]")
